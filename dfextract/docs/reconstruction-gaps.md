...
| World → still sprite xy | Scripts place actors in SET units (`actorstar` / `actorxyz`). A **star** is a named SET pin; `town.leroy1` is (1740, 3536), slot B of `town.leroy2`. Engine tiles are **256** (`DF.EXE` `shl ax,8; add ax,0x80` at `0x40ddac`) — 255 was 0–255 counted as the span. CST blit is the DFET hotspot (256, 192) + header `pos_x`/`pos_y`, scaled — not bbox-center. Screen **X** is DF.EXE `0x40dcd0`: `256 + 310 * right / forward` (focal `0x136` at `0x40d255`; TRIG / 16384). Lens set back **64** from tile-center feet (SET header +24 on every map; `0x40e081`). O7 N Leroy still-x **354**; original midline **353**. 90° pinhole from 255-unit feet was 386; 1/z X was ~306. **Y and scale** use `256/(256+forward)` from the feet with near plane **248** so the hotspot stays in Z=5. Engine Y `viewH/2 + camZ * 310 / forward` (town camZ = SET +26 = 62) sits in Z=4 and clips feet. O7 east does not draw him. Do not freeze XY on in-place turns. Do not scan Z for Y. Do not nudge stars. |
