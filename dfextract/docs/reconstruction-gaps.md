...
| Z-buffers | **Decoded and used at runtime.** Trailing RLE after SET color stills. Offsets are from the Z table start (first offset = `height*2`). `python cli.py --type set --z` writes `FRAMES/z/*.png` and does **not** rewrite color stills. South-gate road Z is **3** at your feet … **7** up the street (24 = sky). Play blits CST pixels when `actorZ <= stillZ` (smaller = closer). Actor Z is `round(nearZ / persp)` with nearZ sampled at the bottom of the still (3 on the south gate), then at most **one** plane closer if the hotspot is dirt (Help Z=5 on Z=4). Unconditional `min(hotspot Z)` put Leroy through buildings on the range road. EXE sprite Z is `(forward − zclip − setback + 128) >> 6` (`0x41535c`); play does not use that. `actorzclip` is stored (stdactor 32) but is not a 24-level subtract — 32 would punch everyone through the fence. |
| World → still sprite xy | Scripts place actors in SET units (`actorstar` / `actorxyz`). A **star** is a named SET pin; `town.leroy1` is (1740, 3536), slot B of `town.leroy2`. Engine tiles are **256** (`DF.EXE` `shl ax,8; add ax,0x80` at `0x40ddac`). CST blit is the DFET hotspot (256, 192) + header `pos_x`/`pos_y`, scaled. Screen **X** is DF.EXE `0x40dcd0` (CST `0x415213` / PRP `0x428173`): `256 + 310 * right / forward` (focal `0x136`; lens setback SET +24 = 64). O7 N Leroy still-x **354**. **Y** is 1/z from the feet so the hotspot stays in SET Z=5. Engine pinhole Y (`132 − 310*(objZ−camZ)/forward`, camZ=62) is traced and unused — it hid the N7 jug and walked Leroy down the still. Filmstrip camera `0x40dd90` walks `index*64` / turns `index*16`; play lerps XY on walks and freezes in-place-turn yaw. Do not scan Z for Y. Do not nudge stars. |
| CST idle / `makeloop` | **Wired from scripts.** One-shot timer in `60/framerate` frames (boot `framerate (3)` → 20 Hz). Leroy `leroyidle`: 8% drink, else turn to camera if `realdist < hotdist`, else `actordeg + 2`. `cameraxyz` is one tile behind the standing point on the view axis (`playerxyz` stays the tile center) so `calcdeg` faces the lens. Camera pose changes re-arm standing idle loops. Drink CST strip is 32 frames (8×4); each pose is held **6 script frames** (`toidle` 25 / 4). Walk poses advance on the engine tick from CST setInfo +0x2e (Leroy 16 slots, two ticks per pose). `endturn` fires when a turn completes (Cast default). Do not invent extra fidgets. |
...
| Camera / FOV / 512×264 vs 512×384 | Scripts use raw `pointx` numbers. Outdoor stills are 512×264 (SET header +42/+44). Play X uses DF.EXE focal **310** (`0x136`), not 90° on 512. Dest-rect C recovered at `0x40dcd0`. Full writeup + dead ends: [`src/play/README.md`](../../src/play/README.md) § World → still. |
