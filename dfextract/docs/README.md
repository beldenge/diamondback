...
| [images.md](images.md) | Palette, sprites, SET/MOV stills (sizes, 255/L7/skull). **Pal 0 vs codec skip 255** (white salt on doors / gun / skeletons) |
...
`out/` is generated by `cli.py`. **Do not edit it.** Wrong decode → fix
`dfextract/` and re-extract. Wrong playback of a faithful dump → fix
`src/`. Pretty-printed `.txt` and runtime `.json` are both generator
output.
