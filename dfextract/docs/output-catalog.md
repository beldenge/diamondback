...
| `timeline.json` | When the v1 80-byte table parses (`--scripts` is enough; also `--video` / `--frames`). `tick_hz` 60, per-still `hold_ticks` / `start_tick` / `wait` / optional `wait_audio` (rec+0x1A bit 0: hold until group-A mixer idle), `hotspots` (type-2 dest+channel; type-4 nested `movie`), clips with `channel` (`A1`… / `B`) and `duration_ticks` (PCM length), optional `next` (rec+0x16==3 Pascal, e.g. `towerup` → `towertop.mov`). `duration_seconds` is the still table, not wait_audio wall-clock. `python movplay.py <this folder>` uses it with `FRAMES/` + `AUDIO/` (no mp4). Linear movplay does **not** follow `next`, type-4, or `wait_audio`; in-game `playmovie` does. |
