...
| Menu / save | Skull HUD (`horn`) is extracted `gotoflat (4)` → NEW.FLT `score`. Save/Open/Quit/Credits/volume/keys are FLT buttons, not a remake File menu. HUD overlays share `visualeffect (barndooropen, 30)` / `barndoorclose`: map `gotoflat (2)`, inventory `gotoflat (3)`, skull `gotoflat (4)`. That is DF.EXE `FUN_0040eec0`: 8 vertical strips (`width/16+1` = 33px) growing from the center, `n/8` ticks each. Close is `FUN_0040edf0` (edges inward). Do not skip that wait — the delay *is* the wipe. Clip the painted `#play-flat-visual`, not `#play-flat` — Subtitles is right of center (x 298–408) and `clip-path` on the hit root ate those clicks. During the wipe `#play-flat` background is transparent so the town still shows in the doors. `help.mov` is a 512×384 wait still; the OK hotspot (382,314)–(466,349) uses the touch cursor. `savegame` is a `.json` file picker (`showSaveFilePicker` on Chromium; Firefox/Safari download `day-N-night.json` and a “Saved … to your downloads” notice). `opengame` is file-only (`.json` and old `.rtd` JSON downloads). `localStorage` is one autosave (`diamondback.autosave`): Continue (`&continue=1`) and a **document reload of the play URL** restore it. When an autosave exists the Resurrected card shows Continue and New Game; the card art is Continue. New Game confirms, then boots without loading (browser progress is replaced; files you saved are not). Do not treat PerformanceNavigationTiming `reload` as resume for in-page card clicks — an F5 of the title leaves that type sticky, which used to load the autosave from the card. After Open, that autosave is the loaded timeline. Unlocked does not share it. Death uses the `death` flat (New / Open / Quit). |

Later days are the same `advanceday` / `canadvance` / SET scripts, not a second plot. Day 2 morning shops follow `lock*` (`phase < 2` still knocks). Two of gun/boots/bullets (`day2items > 1`) plus street `triggerx` walkabout → `d2md2a.mov`. Day 2 night is jail (`d2ad2n.mov`). Kid G5 `kiddie.mov` is the extracted walk-in, then `advanceday`. Day 3 item gates are Jones’s ring + `pages`, then mask/book/flute. Day 3 night sleep needs `day3bedtime` (`tbird` + `tstone` + `phase > 3`). Day 4 afternoon is extracted `openfight` (not Unlocked `sandboxfight`). Day 4 night fountain is extracted (`tstone` in the box) — Unlocked still fakes that lock. Day 5 G14 south is the five-ending puppet chain (`trottend` / `mayorend` / `marieend` / `yunniend` / `deserend`) then `credits.flt` `scrollnames`. `/?mode=resurrected&continue=1` skips boot `advanceday` and restores the autosave (so does a refresh of the play page; New Game on the title does not). Blob `format` is 1. Story playback is not fully proven — a breaking snapshot change should bump `SAVE_FORMAT` (no migration reader yet). Code: [`save.ts`](save.ts). Tests: [`save.test.ts`](save.test.ts), [`story.test.ts`](story.test.ts).
...
| Linear playmovie abort on rec+0x16==1 | DESEREND listed 0:45 then stopped at the chest still. Kind 1 is a scene end in a concatenated reel (INTRO3 / day-change too). Town goodbye is the next scene in `deserend.mov`, not `finalend.mov` (other `*end` reels chain Finale; desert does not). Honor kind 1 only on a hotspot dest segment. |
| Group A only on exact rec start | DESEREND waving was silent. Rec+32 is rec 807; extract holds the cue to 809 (previous scene A4 still ending). 809 sits between stills 807 and 819. Fire A in `[recStart, recEnd)` like a B head. Continuation B still chains on buffer end. |
...
| Picture Show `voices.stop()` when switching reels | Speech-only. The first movie’s A/B (and scheduled B joins) kept playing into the next. `stopAllFx` plus an fx generation so an in-flight `playFx` cannot start after teardown. |
